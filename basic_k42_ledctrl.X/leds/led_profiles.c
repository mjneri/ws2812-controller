...
extern volatile bool longTmrFlag;
...
void cometsTail(uint8_t tailLen)
{
    ...
    while(!longTmrFlag)
    {
        ...
        for(i=0; i < frames && longTmrFlag == false; i++)
        {
...
void theaterChase(uint8_t szLight, uint8_t szSpace)
{
    ...
    while(!longTmrFlag)
    {
        for(i=0, p=0+ledOffset; i < LEDSTRIPSIZE && longTmrFlag == false; i++, p=(p+1)%combinedSegLen)
        {
...
void oneColorPulse(void)
{
    ...
    while(!longTmrFlag)
    {
        for(i = 0; i < 64 && longTmrFlag == false; i++)
        {
            RGB_ALLSetColor(LEDSTRIPSIZE, RGB_TO_VAL(i,0,0));
            __delay_ms(10);
        }

        for(i = 64; i >= 0 && longTmrFlag == false; i--)
        {
            RGB_ALLSetColor(LEDSTRIPSIZE, RGB_TO_VAL(i,0,0));
            __delay_ms(10);
        }
